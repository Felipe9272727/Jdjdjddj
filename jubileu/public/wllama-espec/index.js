...
var Debug = {
   ...
   */
  decodeFuncIds: (funcIds, isCompatBuild) => __async(null, null, function* () {
   ...
   */
  decodeStackTrace: (stack, isCompatBuild) => __async(null, null, function* () {
...
var isMmproj = (blob) => __async(null, null, function* () {
...
var prepareBlobs = (blobsInp) => __async(null, null, function* () {
  const blobs = [];
  let blobMmproj = null;
...
var isSupportMultiThread = () => ((e) => __async(null, null, function* () {
  try {
...
var isSupportExceptions = () => __async(null, null, function* () {
...
var isSupportSIMD = () => __async(null, null, function* () {
  return WebAssembly.validate(
    new Uint8Array([
      0,
      97,
...
var checkEnvironmentCompatible = () => __async(null, null, function* () {
...
var ProxyToWorker = class {
  ...
  constructor(resources, nbThread, suppressNativeLog, logger, modelLoadActivityCallback, fastModelLoad) {
    __publicField(this, "resources");
    __publicField(this, "logger");
    __publicField(this, "suppressNativeLog");
    __publicField(this, "modelLoadActivityCallback");
    __publicField(this, "fastModelLoad");
    ...
    this.fastModelLoad = fastModelLoad;
  ...
  normalizedChunkBytes() {
    var _a;
    const requested = (_a = this.fastModelLoad) == null ? void 0 : _a.chunkBytes;
    if (!Number.isFinite(requested) || (requested != null ? requested : 0) <= 0) {
      return DEFAULT_FILE_WRITE_CHUNK_BYTES;
    }
    return Math.max(
      1024 * 1024,
      Math.min(64 * 1024 * 1024, Math.floor(requested))
    );
  }
  /**
   * Resolve the exact OPFS entry written by CacheManager. FileSystemFileHandle
   * is structured-cloneable; the synchronous access handle is deliberately
   * created later inside the llama worker, where that API is available.
   */
  openFastModelHandle(ggufFiles) {
    return __async(this, null, function* () {
      var _a, _b;
      if (!this.fastModelLoad || ggufFiles.length !== 1) return null;
      if (typeof navigator === "undefined" || !((_a = navigator.storage) == null ? void 0 : _a.getDirectory) || !((_b = globalThis.crypto) == null ? void 0 : _b.subtle)) {
        return null;
      }
      try {
        const url = this.fastModelLoad.cacheURL;
        const digest = yield crypto.subtle.digest(
          "SHA-1",
          new TextEncoder().encode(url)
        );
        const hash = Array.from(new Uint8Array(digest)).map((byte) => byte.toString(16).padStart(2, "0")).join("");
        const cacheName = `${hash}_${url.split("/").pop()}`;
        const root = yield navigator.storage.getDirectory();
        const cacheDir = yield root.getDirectoryHandle("cache");
        const handle = yield cacheDir.getFileHandle(cacheName);
        const file = yield handle.getFile();
        if (file.size !== ggufFiles[0].blob.size) {
          throw new Error(
            `cached GGUF size mismatch (${file.size} != ${ggufFiles[0].blob.size})`
          );
        }
        return handle;
      } catch (err) {
  ...
  moduleInit(ggufFiles) {
    return __async(this, null, function* () {
      const fastModelHandle = yield this.openFastModelHandle(ggufFiles);
      this.useAsyncFile = fastModelHandle ? false : canUseAsyncFileRead(this.resources.compat);
      ...
      if (!this.useAsyncFile) {
        for (let index = 0; index < nativeFiles.length; index++) {
          const file = nativeFiles[index];
          if (index === 0 && fastModelHandle) {
            try {
              yield this.fileWriteFromOpfs(
                file.id,
                fastModelHandle,
                file.blob.size
              );
          ...
          yield this.fileWrite(file.id, file.blob);
        }
      }
      return res;
    });
  ...
  fileWrite(fileId, blob) {
    return __async(this, null, function* () {
      const chunkBytes = this.normalizedChunkBytes();
      for (let offset = 0; offset < blob.size; offset += chunkBytes) {
        const end = Math.min(blob.size, offset + chunkBytes);
        const value = new Uint8Array(yield blob.slice(offset, end).arrayBuffer());
        yield this.pushTask(
          {
            verb: "fs.write",
  ...
  /** Direct OPFS -> worker HeapFS copy, followed by zero-copy mmap in WASM. */
  fileWriteFromOpfs(fileId, handle, total) {
    return __async(this, null, function* () {
      var _a;
      const result = yield this.pushTask({
        verb: "fs.write_opfs",
        args: [fileId, handle, total, this.normalizedChunkBytes()],
        callbackId: this.taskId++
      });
      if ((result == null ? void 0 : result.writtenBytes) !== total) {
        throw new Error(
          `OPFS short read (${(_a = result == null ? void 0 : result.writtenBytes) != null ? _a : 0} != ${total})`
        );
      }
    });
  }
...
` + decoded);
    ...
    if (verb === MODEL_LOAD_ACTIVITY_EVENT) {
      this.reportModelLoadActivity((_a = args == null ? void 0 : args[0]) != null ? _a : {});
      return;
    }
...
function openWritable(fileName) {
  return __async(this, null, function* () {
    ...
    return {
      truncate: () => __async(null, null, function* () {
      }),
      write: (value) => workerExec({ action: "write", buf: value }),
      close: () => __async(null, null, function* () {
