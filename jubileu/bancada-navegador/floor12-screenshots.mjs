...
async function assertCaptionLayout(page, selector, viewport) {
  ...
  const box = await page.evaluate((selector) => {
    const c = document.querySelector("canvas")?.getBoundingClientRect();
    const d = document.querySelector(selector)?.getBoundingClientRect();
    return c && d ? { bottom: c.bottom, top: d.top, height: c.height } : null;
  }, selector);
  assert(box && box.bottom <= box.top + 1 && box.height > viewport.height * .45,
    selector + ": caption must stay outside the 3D scene");
}
async function runVictoryContract(page, orientation, viewport, diagnostics) {
  await page.getByText("ATIRE NA BOCA!", { exact: true }).waitFor({ state: "visible", timeout: 30000 });
  await page.evaluate(() => {
    const s = window.__f12estado;
    s.nave.piscando = 999;
    for (let i = 0; i < 40; i++) s.projeteis.push({
      id: 100000 + i, tipo: "tiro", x: 0, y: 3.7, z: -24.9,
      vx: 0, vy: 0, vz: 0, r: .48, t: 0, de: "jogador", carregado: true
    });
  });
  await page.waitForFunction(() => window.__f12estado.fase === "queda", null, { timeout: 10000 });
  await assertCaptionLayout(page, '[data-testid="f12-victory-caption"]', viewport);
  assert(await page.locator('[data-testid="f12-dialogue"]').count() === 0, "Victory dialogue started before cinematic");
  const emitted = await page.evaluate(() => window.__f12estado.arma.emitted);
  await page.touchscreen.tap(viewport.width / 2, viewport.height / 2);
  for (const [label, second] of [["instability", 1.5], ["rupture", 3.6], ["fall", 5.8], ["escort", 8.8]]) {
    await page.waitForFunction(t => window.__f12estado.cinema >= t, second, { timeout: 30000 });
    const state = await page.evaluate(() => ({
      phase: window.__f12estado.fase, t: window.__f12estado.cinema,
      projectiles: window.__f12estado.projeteis.length,
      emitted: window.__f12estado.arma.emitted,
      headVisible: window.__f12cena.getObjectByName("cabeca")?.visible
    }));
    assert(state.phase === "queda" && state.projectiles === 0 && state.emitted === emitted,
      orientation + ": gameplay advanced during cinematic");
    diagnostics.push({ step: "victory-" + label, ...state });
    await page.screenshot({ path: outputDir + "/" + orientation + "-victory-" + label + ".png" });
  }
  await page.waitForFunction(() => window.__f12estado.fase === "vitoria", null, { timeout: 30000 });
  await assertCaptionLayout(page, '[data-testid="f12-dialogue"]', viewport);
  assert(await page.evaluate(() => window.__f12cena.getObjectByName("cabeca").visible === false), "Boss resurrected after the fall");
  for (let i = 0; i < 2; i++) await page.locator('[data-testid="f12-dialogue"] button').tap();
  await page.waitForFunction(() => window.__f12estado.fase === "despedida");
  for (let i = 0; i < 3; i++) await page.locator('[data-testid="f12-dialogue"] button').tap();
  await page.locator("button").filter({ hasText: "ANDAR 12" }).first().tap();
  await page.waitForFunction(() => window.__f12estado.fase === "intro" &&
    window.__f12estado.vida === 240 && window.__f12estado.projeteis.length === 0 &&
    window.__f12estado.cinema === 0, null, { timeout: 10000 });
  diagnostics.push({ step: "victory-exit-reenter", state: await readState(page) });
}
  ...
  for (const [orientation, viewport] of viewports) {
    ...
    try {
      ...
      for (const [name, atMs] of introMilestones) {
        ...
        await assertCaptionLayout(page, '[data-testid="f12-intro-caption"]', viewport);
      ...
      await page.evaluate(() => { window.__f12estado.nave.piscando = 999; });
      await runTouchContract(page, orientation, viewport, diagnostics);
      await runVictoryContract(page, orientation, viewport, diagnostics);
