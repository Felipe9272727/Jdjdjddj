...
export const VisualJoystick = ({ x, y, active, origin }: any) => {
  ...
  return (
    <div className="absolute pointer-events-none z-50" style={{ left: origin.x, top: origin.y, transform: 'translate(-50%, -50%)' }}>
      <div className="w-24 h-24 sm:w-32 sm:h-32 rounded-full bg-white/10 ring-2 ring-white/20 backdrop-blur-sm flex items-center justify-center">
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 justify-end pb-4 px-3 md:px-[20%] landscape:pb-2 landscape:px-8 max-h-[100dvh] overflow-hidden">
        ...
        {/* Dialogue Text Box */}
        <div className="bg-black/80 border-t-2 border-yellow-500/50 p-3 sm:p-4 md:p-6 shadow-2xl pointer-events-auto mb-3 sm:mb-4 rounded-t-xl landscape:mb-2">
          <p className="text-white text-sm sm:text-base md:text-xl font-bold font-serif leading-relaxed min-h-[3rem] sm:min-h-[4rem] text-shadow-sm">
