...
const Avatar = ({ animation, visible = true, pickupTrigger = 0 }: {
  ...
  const PICKUP_HOLD = 0.5;
  const PICKUP_RETRACT = 0.4;
  ...
  // Find arm bones for pickup animation
  useEffect(() => {
    ...
    if (arm) { armOrigRotRef.current.copy(arm.rotation); console.log('[Avatar] Found arm bone:', arm.name); }
    if (forearm) { forearmOrigRotRef.current.copy(forearm.rotation); }
  }, [scene]);
  ...
  useEffect(() => {
    if (pickupTrigger !== lastPickupTriggerRef.current && pickupTrigger > 0) {
      ...
      console.log('[Avatar] Pickup triggered:', pickupTrigger);
    }
  }, [pickupTrigger]);
  ...
  useFrame((_, dt) => {
    ...
    pickupTimeRef.current += Math.min(dt, 0.05);
    const t = pickupTimeRef.current;

    if (t >= PICKUP_TOTAL) {
      pickupTimeRef.current = -1;
      if (armBoneRef.current) armBoneRef.current.rotation.copy(armOrigRotRef.current);
      if (forearmBoneRef.current) forearmBoneRef.current.rotation.copy(forearmOrigRotRef.current);
    ...
    if (armBoneRef.current) {
      armBoneRef.current.rotation.x = armOrigRotRef.current.x - ext * 1.2;
      armBoneRef.current.rotation.z = armOrigRotRef.current.z - ext * 0.15;
    }
    if (forearmBoneRef.current) {
      forearmBoneRef.current.rotation.x = forearmOrigRotRef.current.x - ext * 0.6;
    }
