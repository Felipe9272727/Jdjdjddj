...
// event. Multiplying them by frame dt made desktop sensitivity vary wildly
// between 30/60/120 fps; consume them once in radians instead.
...
function firstPersonBaseFov(aspect: number, level: number): number {
  if (level === 2) return aspect < 0.85 ? 92 : 87; // água pede um pouco mais de periferia
  if (level === 9) return aspect < 0.85 ? 87 : 80; // olho baixo do Fiapo, sem lente olho-de-peixe
  ...
  useFrame((state, dt) => {
    ...
    if (dialogueOpen && dialogueFocusRef?.current && currentLevel !== 6) {
        ...
        if (currentLevel === 2) {
            ...
            if (kb) { pos.current.z = kb.z; jumpVelYRef.current = kb.vy; playFloor3Hit(); }
        ...
        if (fp) {
            ...
            // Pequeno deslocamento físico, sem mexer na direção da mira. O bob
            // antigo (ou shake em X global) fazia o cenário tremer; aqui toda
            // oscilação acontece no referencial local da câmera.
...
export const FPArmModel: React.FC<FPArmModelProps> = ({ zoomLevel, currentLevel = 0, armExtended, pickupTrigger, active, flashlightActive, flashlightOwned, pickupItem, cutsceneActive }) => {
  ...
  useFrame((_, dt) => {
    ...
    // Floor 3 has its own rubber-hose gloves; Floor 9 has the textured 3D
    // Fiapo rig. Never stack the old human arm over either viewmodel.
    const floorOwnsViewmodel = currentLevel === 3 || currentLevel === 9;
    g.visible = active && fpView && holdingSomething && !cutsceneActive && !floorOwnsViewmodel;   // never let the FP hand block a cutscene
    if (!g.visible) return;
