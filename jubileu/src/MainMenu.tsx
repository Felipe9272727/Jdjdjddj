...
export const MainMenu: React.FC<MainMenuProps> = ({ onPlay }) => {
  ...
  // ─── CREATOR MODE ───
  const [creatorModeOpen, setCreatorModeOpen] = useState(false);
  // ─── CREATOR MODE ───
     ...
     if (multiplayerEnabled && !user) {
         ...
         try {
             ...
             if (e.code === 'auth/unauthorized-domain') {
                 setLoginError("Connection blocked. This domain isn't authorized in Firebase.");
             } else if (e.code === 'auth/popup-closed-by-user') {
                 setLoginError("Login cancelled. The door closed before you could enter.");
             } else {
                 setLoginError("Could not connect: " + e.message);
             }
             return;
         }
         setIsLoggingIn(false);
      ...
      <div className="absolute inset-0 z-50 overflow-hidden bg-black">
        ...
        <div className={`absolute inset-0 z-20 flex flex-col transition-all duration-1000 ${showContent ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="md:hidden flex flex-col h-full pointer-events-auto">
            ...
            {/* ─── CREATOR MODE: mobile panel ─── */}
            {creatorModeOpen ? (
              <div className="flex-1 flex items-center justify-center px-4 min-h-0 overflow-y-auto scrollbar-hide">
                <CreatorMode
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-6 min-h-0 overflow-y-auto landscape:justify-center scrollbar-hide">
              ...
              <div className="text-center mb-3 sm:mb-6 landscape:mb-4 shrink-0">
                <p className="text-amber-500/50 text-xs uppercase tracking-[0.5em] mb-2 font-light landscape:hidden">Welcome to</p>
              ...
              <p className="text-white/60 text-center text-xs sm:text-sm max-w-xs leading-relaxed landscape:hidden mb-2">
                A liminal, interactive experience.<br/>
                <span className="text-amber-500/55">Please remain calm.</span>
              </p>
              ...
              <div className="hidden landscape:block w-full max-w-xs mt-2">
                <div className="flex flex-col gap-2 mb-4 bg-black/50 border border-white/10 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                     ...
                     <button onClick={() => setMultiplayerEnabled(m => !m)} aria-pressed={multiplayerEnabled} aria-label={multiplayerEnabled ? 'Disable multiplayer' : 'Enable multiplayer'} className={`w-12 h-7 min-h-[28px] rounded-full transition-colors relative ring-1 ring-white/20 ${multiplayerEnabled ? 'bg-amber-500' : 'bg-gray-800'}`}>
                  ...
                  {multiplayerEnabled && (
                      <>
                          <div className="mt-2">
                              <label className="text-white/65 text-xs tracking-wider mb-1.5 block font-medium">Your Name</label>
                              <input
                                  type="text"
                                  value={playerName}
                ...
                >
                  ...
                  <div className="relative bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 rounded-lg">
                    ...
                    <div className="relative flex items-center justify-center gap-3 px-8 py-4 text-black font-bold text-base tracking-widest">
                      <span className="group-hover:tracking-[0.3em] transition-all duration-300">ENTER</span>
            ...
            {/* ─── CREATOR MODE: end mobile panel ─── */}
            
            {/* ─── CREATOR MODE: back button when open on mobile ─── */}
            {creatorModeOpen ? (
              <div className="p-3 pb-[max(1rem,env(safe-area-inset-bottom,0px))] sticky bottom-0 bg-gradient-to-t from-black via-black/95 to-transparent z-40 landscape:hidden shrink-0">
                <button
                  onClick={() => setCreatorModeOpen(false)}
                  className="w-full py-3 rounded-lg border border-white/20 bg-white/5 text-white/70 text-sm font-bold tracking-wider hover:bg-white/10 transition-colors"
                >
                  VOLTAR
                </button>
              </div>
            ...
            <div className="p-3 pb-[max(1rem,env(safe-area-inset-bottom,0px))] sticky bottom-0 bg-gradient-to-t from-black via-black/95 to-transparent z-40 landscape:hidden shrink-0">
              <div className="flex flex-col gap-2 border border-white/10 rounded-lg mb-2 bg-black/50 backdrop-blur-md p-2.5">
                 <div className="flex items-center justify-between">
                     <span className="text-white/70 text-xs tracking-widest uppercase font-medium">Multiplayer</span>
                     <button onClick={() => setMultiplayerEnabled(m => !m)} aria-pressed={multiplayerEnabled} className={`w-12 h-7 min-h-[28px] rounded-full transition-colors relative ring-1 ring-white/20 ${multiplayerEnabled ? 'bg-amber-500' : 'bg-gray-800'}`}>
                        <div className={`absolute top-1 left-0.5 w-5 h-5 rounded-full bg-white transition-transform ${multiplayerEnabled ? 'translate-x-5' : 'translate-x-0'}`} />
                     </button>
                 </div>
              ...
              {/* ─── CREATOR MODE: mobile bottom creator button ─── */}
              <button
                onClick={() => setCreatorModeOpen(true)}
                className="w-full mt-2 flex items-center justify-center gap-1.5 border border-purple-400/30 bg-purple-500/10 py-2.5 rounded-xl text-sm text-purple-300 font-bold hover:bg-purple-500/20 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.1-5.1a1 1 0 010-1.41l.7-.71a1 1 0 011.42 0l3.54 3.54 7.78-7.78a1 1 0 011.42 0l.7.71a1 1 0 010 1.41l-9.2 9.2a1 1 0 01-1.4.14z" />
                </svg>
                MODO CRIADOR
              </button>
              {/* ─── CREATOR MODE: end mobile bottom creator button ─── */}
              
              <div className="flex justify-center gap-5 mt-3 text-white/65 text-xs">
          ...
          <div className="hidden md:flex h-full items-center justify-center pointer-events-auto">
            {/* ─── CREATOR MODE: when open, show the panel centered ─── */}
            {creatorModeOpen ? (
              <div className="flex-1 max-w-lg mx-auto px-8 flex flex-col justify-center">
                <div className="mb-6">
                  <button
                    onClick={() => setCreatorModeOpen(false)}
                    className="flex items-center gap-2 text-white/50 hover:text-white/80 text-sm transition-colors mb-6"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <div className="flex items-center gap-10 lg:gap-24 max-w-6xl mx-auto px-8">
              ...
              <div className="flex-1 max-w-lg flex flex-col justify-center">
                ...
                <div className="mb-6 animate-fade-in-up" style={{ animationDelay: '0.8s' }}>
                  <p className="text-amber-500/40 text-sm uppercase tracking-[0.5em] mb-2 font-light">Welcome to</p>
                ...
                <div className="animate-fade-in-up mt-2" style={{ animationDelay: '1s' }}>
                  <div className="flex flex-col gap-3 p-4 border border-white/10 rounded-xl mb-4 bg-black/50 backdrop-blur-sm max-w-sm">
                     <div className="flex items-center justify-between">
                         <span className="text-white/75 text-xs tracking-widest uppercase">Multiplayer (Online)</span>
                         <button onClick={() => setMultiplayerEnabled(m => !m)} aria-pressed={multiplayerEnabled} className={`w-12 h-6 rounded-full transition-colors relative border border-white/20 ${multiplayerEnabled ? 'bg-amber-500' : 'bg-gray-800'}`}>
                  ...
                  <button onClick={handlePlay} disabled={isLoggingIn} className="group relative overflow-hidden rounded-xl transition-all duration-500 hover:scale-[1.02] active:scale-[0.98]">
                    ...
                    <div className="relative bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-500 rounded-xl">
                      ...
                      <div className="relative flex items-center gap-4 px-8 py-4 text-black font-bold text-lg justify-center">
                        <span className="tracking-[0.2em] group-hover:tracking-[0.3em] transition-all duration-500">{isLoggingIn ? "CONNECTING..." : "ENTER THE LOBBY"}</span>
                        {!isLoggingIn && (
                  ...
                  {/* ─── CREATOR MODE: desktop button ─── */}
                  <button
                    onClick={() => setCreatorModeOpen(true)}
                    className="w-full mt-3 flex items-center justify-center gap-2 border border-purple-400/30 bg-purple-500/10 py-2.5 rounded-xl text-sm text-purple-300 font-bold hover:bg-purple-500/20 transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17l-5.1-5.1a1 1 0 010-1.41l.7-.71a1 1 0 011.42 0l3.54 3.54 7.78-7.78a1 1 0 011.42 0l.7.71a1 1 0 010 1.41l-9.2 9.2a1 1 0 01-1.4.14z" />
                    </svg>
                  ...
                  {/* ─── CREATOR MODE: end desktop button ─── */}
                  
                  <div className="flex gap-8 mt-6 text-white/50 text-sm">
                    <span className="flex items-center gap-2"><kbd className="px-2 py-1 bg-white/5 border border-white/10 rounded text-xs font-mono">WASD</kbd> Move</span>
                    <span className="flex items-center gap-2"><kbd className="px-2 py-1 bg-white/5 border border-white/10 rounded text-xs font-mono">MOUSE</kbd> Look</span>
                    <span className="flex items-center gap-2"><svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728" /></svg> Headphones</span>
                  </div>
                </div>
              </div>
            </div>
            )}
            {/* ─── CREATOR MODE: end desktop layout ─── */}
          </div>
          
          <div className="absolute top-6 left-6 text-white/60 text-xs tracking-wider hidden md:flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />SISTEMA ATIVO
          </div>
          <div className="absolute top-6 right-6 text-white/60 text-xs tracking-wider hidden md:flex items-center gap-2">
            <span className="text-red-500">●</span> REC<span className="ml-4">CAM_01</span>
          </div>
          <div className="absolute bottom-6 left-6 text-white/60 text-xs tracking-wider hidden md:block">&copy; 2026 LIMINAL SYSTEMS</div>
          <div className="absolute bottom-6 right-6 text-white/60 text-xs tracking-wider hidden md:block cursor-default select-none" onClick={() => setEasterEgg(p => p + 1)}>
            {easterEgg >= 3 ? (
