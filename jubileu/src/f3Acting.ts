...
export function createF3ActingLayer() {
  ...
  function apply(bones: THREE.Bone[], input: ActingInput): ActingCue {
    ...
    const body = bones[B.body], head = bones[B.head];
    const right = bones[B.r_arm], left = bones[B.l_arm];
    for (const bone of [body, head, right, left]) {
      if (!bone) continue;
    ...
    if (input.scene === 'intro') {
      ...
      switch (input.phase) {
        ...
        case 'lean':
        case 'taunt':
          body.rotation.z += .055 * accent + .025 * settle;
          head.rotation.z -= .10 * accent + .04 * settle;
          left.rotation.x += .12 * accent;
