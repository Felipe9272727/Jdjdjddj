/**
 * shaders.ts — Custom ShaderMaterial definitions
 *
 * Uses drei's `shaderMaterial` helper. Each returns a constructor
 * that must be instantiated with `new (XxxMaterial as any)()`.
 */
...
export const WaterMaterial = shaderMaterial(
    ...
    /* glsl vertex */ `
      ...
      vec4 gerstner(vec2 pos, vec2 dir, float steepness, float wavelength, float t) {
        ...
        float sinF = sin(f);
        float cosF = cos(f);
        return vec4(-dir.x * a * cosF, a * sinF, -dir.y * a * cosF, 0.0);
      }
...
export const CausticsMaterial = shaderMaterial(
    ...
    /* glsl fragment */ `
      ...
      void main() {
        ...
        vec3 col = vec3(c * 0.08, c * 0.18, c * 0.14);
